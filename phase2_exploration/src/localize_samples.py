...
def confirmed_entry(name: str, process: str, role: str, file_name: str) -> dict[str, object]:
    ...
    expected_size = int(OFFICIAL_RECORDS[name]["distribution_file_size_bytes"])
    exists = path.exists()
    ...
    return {
        ...
        "process": process,
        "role": role,
        "source_urls": [WORKSHOP_SOURCE, OFFICIAL_RECORDS[name]["record_url"]],
        ...
        "notes": (
            "Validated local reduced ROOT sample for current processing use. Local Events tree entries are reduced/skimmed processing entries and are not used as normalization denominators."
...
def build_manifest() -> dict[str, object]:
    ...
    return {
        ...
        "policy": "Use the local reduced 2012 CMS Open Data ROOT samples under data/ and mc/ for the mu tau_h H to tau tau analysis; no remote sample access is performed by this manifest task.",
        "sources": {
...
def main() -> None:
    ...
    failed = [item["sample"] for item in manifest["samples"] if item["status"] in {"failed", "missing-reduced"} and item["analysis_blocking"]]
    validated = [item["sample"] for item in manifest["samples"] if item["status"] == "local_validated"]
    log.info("Wrote %s", path)
    log.info("Validated %d local reduced samples", len(validated))
    if failed:
